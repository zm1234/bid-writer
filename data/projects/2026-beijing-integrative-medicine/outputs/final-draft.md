...
**第一申请人**：
长期从事医学影像学研究和心血管疾病筛查工作，在国内外期刊发表相关论文20余篇，承担省部级课题3项，具有丰富的心血管影像研究经验。
...
- UK Biobank研究团队利用深度学习方法对颈动脉超声图像进行分析，开发了基于YOLOv8的自动斑块检测模型，在大规模人群（19,499人）中验证了模型的性能，准确率达到89%以上。
- 欧洲BiDirect研究进一步验证了深度学习模型在不同超声设备上的泛化能力。

**国内研究现状**：

我国在医学人工智能领域发展迅速，但在颈动脉超声自动检测方面的研究相对较少。目前国内颈动脉斑块检测仍主要依赖人工判读，缺乏成熟的自动化检测系统。
...
（2）数据支撑
- 昆山队列：60万张颈动脉超声影像（合作提供）
- UK Biobank：19,499人颈动脉超声数据（已获取）
- BiDirect：2,105人验证数据（可获取）

（3）硬件资源
- GPU服务器（NVIDIA QUADRO RTX 5000）
- 存储设备
- 开发工作站

### 2. 本课题涉及的主要科研设备、仪器、试剂
...
- **理论可行性**：深度学习在医学影像分析已取得成功案例
- **技术条件可行性**：团队具备相关技术能力，有足够算力支撑
- **操作可行性**：与昆山队列团队合作，数据获取有保障
- **经济可行性**：15万元经费可满足项目需求
